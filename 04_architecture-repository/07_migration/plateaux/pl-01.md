---
domain: plateaux
id: PL-01
type: plateau
title: Plateau 1 — Socle & confiance numérique
status: draft
owner: Direction du Numérique en Santé
version: "0.1"
envelope: 02_artsn/09_feuille-de-route/pl-01-plateau-1-socle-confiance-num-rique.md
realizes: ["ABB-IDENTITE-BENEFICIAIRE", "CAP-17"]
maps_to: ["CAP-01", "CAP-02", "CAP-04", "CAP-07", "CAP-14", "CAP-17"]
precedes: ["PL-02"]
related: ["ABB-IDENTITE-BENEFICIAIRE", "CAP-17", "CMP-32", "CMP-39", "WP-01"]
tags: ["artsn", "plateau", "pl-01", "feuille-de-route"]
---
# Plateau 1 — Socle & confiance numérique

Le plateau 1 est l'état cible où l'infrastructure est opérationnelle, la sécurité de confiance en place, la gouvernance de données publiée et les référentiels de base disponibles. Il est la condition préalable à tout service orienté usager.

Il [couvre la capacité « Socle technique et sécurité »](../../05_building-blocks/abb/abb-identite-beneficiaire.md) et la [capacité « Engagement patient et identité numérique »](../../02_architecture-elements/strategy/capabilities/cap-17.md), et [précède le plateau 2](pl-02.md). Les [paquets de travail L1](../work-packages/wp-01.md) le réalisent.

## État cible (borne temporelle)

Atteint à l'issue des lots qui y contribuent — il s'agit d'un état cible intermédiaire ou final de l'architecture.

## Capacités couvertes

Ce plateau couvre les éléments suivants : [ABB-IDENTITE-BENEFICIAIRE](../../05_building-blocks/abb/abb-identite-beneficiaire.md), [CAP-17](../../02_architecture-elements/strategy/capabilities/cap-17.md).

## Précédence

Il précède le [PL-02](pl-02.md).
