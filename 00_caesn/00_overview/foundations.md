...
Trois référentiels opèrent à des niveaux différents et complémentaires. Ils définissent des principes et modèles d'organisation, sans prescrire de logiciels.

### Cadre Digital Public Infrastructure for Health (DPI-H) de l'OMS

Définit les six composantes d'infrastructure numérique que tout système de santé national doit développer comme **biens communs** : identité numérique des patients et agents, couche d'échange de données, registre des formations sanitaires, terminologie et codification, capacité d'analyse et de visualisation, cadre de confiance-sécurité-consentement.
...
Définit comment organiser les composantes en couches interopérables : couche de médiation (échanges), couche de référence (registres et terminologies), couche de points de service (applications de terrain).

*Répond à :* comment organiser les composantes d'un système cohérent ?

### GovStack Building Blocks

Spécifications des composantes transversales à l'administration publique (identité civile, paiements, échanges inter-administrations, consentement) qui relient le système santé à l'Infrastructure numérique de l'État.

*Répond à :* comment le système santé s'intègre-t-il dans l'État numérique malgache ?
