...
La source de vérité est le champ `implements` du frontmatter de chaque profil dans `04_architecture-repository/profils/pt-XX.md`. Le champ `related` n'est pas repris ici car il documente des dépendances transversales et non des implémentations directes.
...
- Le mapping est dérivé du champ `implements` du frontmatter de chaque profil dans `04_architecture-repository/profils/pt-XX.md`. Le champ `related` n'est pas repris car il documente des dépendances transversales et non des implémentations directes.
